...
function createExternalRoutes() {
  ...
  /**
   * Get system status from ID/BFDB
   */
  async function getStatus() {
    ...
    try {
      const subjectResults = await fetch(baseURL.replace('<DATASET>', 'subjects'), {
        headers: {
          accept: 'application/json',
          'cache-control': 'no-cache'
        },
      ...
      const nameResults = await fetch(baseURL.replace('<DATASET>', 'names'), {
        headers: {
          accept: 'application/json',
          'cache-control': 'no-cache'
        },
      ...
      const names = await nameResults.json();
      const mostRecentName = names.orderedItems[0].object.id;
      const mostRecentNameURL = mostRecentName.replace('id.loc.gov', 'preprod-8080.id.loc.gov') + '.marcxml.xml';

      const recentName = await fetch(mostRecentNameURL, {
        headers: {
      ...
      const subjects = await subjectResults.json();
      const recentNameXML = await recentName.text();

      const pattern = /tag="005">(?<date>.*)<\//;
      const match = recentNameXML.match(pattern);
      ...
      const recentDateTime = new Date(date.replace(
        /^(\d{4})(\d\d)(\d\d)(\d\d)(\d\d)(\d\d)$/,
        '$4:$5:$6 $2/$3/$1'
      ));
      const subjectDate = new Date(subjects.orderedItems[0].object.updated.replace(
        /^(\d{4})(\d\d)(\d\d)$/,
        '$2/$3/$1'
      ));
  ...
  router.get('/status', (req, res) => {
    const updates = { lastUpdateNames, lastUpdateSubjects };
